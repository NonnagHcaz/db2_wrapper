"""File contains meta data for the package."""

from __future__ import absolute_import, print_function, division

__version__ = '1.0.0'
__title__ = 'db2_wrapper'
__summary__ = 'Wrapper to make ibm_db connections simpler.'
__uri__ = 'https://github.com/Gannon93/db2_wrapper'
__author__ = 'Zach Gannon'
__email__ = 'zachgannon93@gmail.com'
__license__ = 'MIT'
__copyright__ = 'Copyright (c) 2016-2017 {0}'.format(__author__)
__classifiers__ = '\n'.join([
    'Natural Language :: English', 'License :: OSI Approved :: MIT License',
    'Programming Language :: Python :: 2',
    'Programming Language :: Python :: 2.7',
    'Programming Language :: Python :: 3',
    'Programming Language :: Python :: 3.3',
    'Programming Language :: Python :: 3.4',
    'Programming Language :: Python :: 3.5',
    'Topic :: Software Development :: Libraries :: Python Modules'
])
